...
// 当调用一个方法时，先不去Class的方法列表中查找，而是先去找cache_t cache
struct cache_t {
...
class list_array_tt {
 ...
 public:
    ...
    // category中的方法，会覆盖class的原始方法，其实并没有真正的覆盖，
    // 由于category的方法被排到了原始方法的前面，所以在消息查找流程中，会返回首先被查找到的category方法的实现。
...
struct class_rw_t {
    ...
    // 下面三个array，可以被runtime扩展，如Category
...
struct class_data_bits_t {
    ...
    // 不仅包含了指针，同时包含了Class的各种异或flag。来说明Class的属性。
    // 当需要取出这些信息时，需要用对应的FAST_ 前缀开头的flag掩码对bits做按位与操作。
    uintptr_t bits;
private:
...
// objc_class 继承自objc_object，表明objc_class也是一个objc_object。
struct objc_class : objc_object {
    // Class ISA;
    // super class 是一个objc_class *指针
    Class superclass;
    cache_t cache;             // formerly cache pointer and vtable
    // Class的核心，其本质是一个可以被Mask的指针类型。根据不同的Mask，可以取出不同的值。
