...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    int i = cats->count;
    bool fromBundle = NO;
    while (i--) { // 依次读取每一个category，将其methods，propery，protocol添加到mlists，proplist，protolist中存储
...
* Locking: runtimeLock must be write-locked by the caller
**********************************************************************/
// 在class 没有调用 realizeClass之前，不是真正完整的类。
static Class realizeClass(Class cls)
{
    runtimeLock.assertLocked();
    ...
    // 最初返回的是const class_ro_t *类型，也就是类的基本信息。在调用realizeClass方法前，Category定义的各种方法，属性还没有附加到Class上，因此只能够返回类的基本信息。
...
# endif
    ...
    // Discover categories. 
    for (EACH_HEADER) {
        // 读取 __objc_catlist section 下所记录的所有category。并存放到category_t *数组中
        category_t **catlist = 
        ...
        for (i = 0; i < count; i++) {
            ...
            // 返回一个objc_class *对象cls。这一步的目的在于找到category对应的类对象cls
            ...
            {
                // 将class 和 对应的category做了一个映射，以class为key可以找到对应的所有category
