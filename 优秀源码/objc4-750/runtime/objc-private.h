...
// 一个联合，表示Class cls或uintptr_t bits类型。多数时间用的是uintptr_t bits。bits是一个64位的数据
// 每一位或几位都表示了关于当前对象的信息。
union isa_t {
    isa_t() { }
...
// runtime中，对象被定义为objc_object结构体，类被定义为objc_class结构体。
// 而objc_class继承自objc_object。因此类可以看做是一类特殊的对象。
struct objc_object {
private:
    // unit联合，可以表示Class类型，表明Object所属的类
    isa_t isa;
