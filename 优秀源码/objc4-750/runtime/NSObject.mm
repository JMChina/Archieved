...
/**
 - 将weak指针的地址location存入到obj对应的weak_entry_t的数组中，用于在obj析构时，通过该数组找到所有其weak指针引用，并将指针指向的地址置为nil。
 - 如果启用了isa优化，则将obj的isa_t的weakly_referenced置为1。置为1的作用主要是为了标记obj被weak引用了，
 当dealloc时，runtime会根据weakly_referenced标志位来判断是否需要查找obj对应的weak_entry_t，并将引用置为nil。
 */
...
storeWeak(id *location, objc_object *newObj)
{
 ...
 retry:
    if (haveOld) {
        // 如果weak_ptr之前弱引用过一个obj，则将这个objc所对应的SideTable取出，赋值给oldTable
        oldObj = *location;
        oldTable = &SideTables()[oldObj];
    } else {
        oldTable = nil; // 如果weak_ptr之前没有弱引用过一个obj，则oldTable = nil
    }
    if (haveNew) { // 如果weak_ptr 要weak引用一个新的obj，则将该obj对应的SideTable取出，赋值给newTable
        newTable = &SideTables()[newObj];
    } else {
        newTable = nil; // 如果weak_ptr不需要引用一个新obj，则newTable = nil
    }

    // 加锁操作，防止多线程中竞争冲突
    SideTable::lockTwo<haveOld, haveNew>(oldTable, newTable);

    // location 应该与 oldObj保持一致，如果不同，说明当前的location已经处理过oldObj 可是又被其他线程所修改
    ...
    if (haveNew  &&  newObj) {
            ...
            !((objc_class *)cls)->isInitialized()) // 如果cls还没有初始化，先初始化，再尝试设置weak
        {
            ...
            previouslyInitializedClass = cls; // 这里记录一下previousInitializeClass，防止改if分支再次进入
    ...
    if (haveNew) { // 如果weak_ptr需要弱引用新的对象newObj
        // 调用weak_register_no_lock方法，将weak_ptr的地址记录到newObj对应的weak_entry_t中
        newObj = (objc_object *)
        ...
        // *location赋值，也就是将weak_ptr直接指向了newObj。可以看到这里并没有将newObj的引用计数+1
        *location = (id)newObj;
    }
    else {
 ...
 */
// 返回Obj自身，但其值已经做了更改(isa_t中的weak_ref位置)
